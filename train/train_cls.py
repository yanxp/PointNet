...
parser.add_argument('--outdir',type=str,default='cls',help='output dir of training')
...
train_dataset = pointdata.PartDataset(root='../data/shapenetcore_partanno_segmentation_benchmark_v0',npoints=opt.num_points,classification=True)
trainLoader = Data.DataLoader(train_dataset,batch_size = opt.batchsize,shuffle=True,num_workers=opt.num_workers)

test_dataset = pointdata.PartDataset(root='../data/shapenetcore_partanno_segmentation_benchmark_v0',npoints=opt.num_points,classification=True,train=False)
...
model = pointnet.PointNetCls(num_points=opt.num_points,k=num_classes)
if opt.model != '':
...
for epoch in range(opt.num_epoch):
	...
	for i,(points,labels) in enumerate(trainLoader):
		...
		totalloss += loss.data
		totalnum += len(labels)
		batchacc = correct.item()/(opt.batchsize)
		trainacc += batchacc
		# print('accuracy:{}'.format(batchacc))
	print('epoch:{},train loss:{},train accuracy:{}'.format(epoch+1,totalloss/totalnum,trainacc/(num_batch)))
	...
	if (epoch+1) % 10 == 0:
		...
		model.train()
	torch.save(model.state_dict(),'%s/cls_model_%d.pth' % (opt.outdir, epoch))
